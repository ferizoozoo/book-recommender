...
import { AuthorEntity, BookEntity, PublisherEntity } from "../library.models.ts";
...
export function mapBookEntityToDomain(bookEntity: BookEntity): Book {
    const book = new Book(
        bookEntity.id,
        bookEntity.title,
        bookEntity.author ? mapAuthorEntityToDomain(bookEntity.author) : new Author(0),
        bookEntity.description,
        bookEntity.isbn,
        bookEntity.publisher ? mapPublisherEntityToDomain(bookEntity.publisher) : new Publisher(0),
        bookEntity.year,
        bookEntity.image,
        bookEntity.rating,
...
export function mapPublisherDomainToModel(publisher: Publisher): PublisherEntity {
    ...
    // Map the user if it exists
    if (publisher.user && publisher.user.id > 0) {
        publisherEntity.user = mapUserDomainToModel(publisher.user);
    }

    return publisherEntity;
}

export function mapPublisherEntityToDomain(publisherEntity: PublisherEntity): Publisher {
    const publisher = new Publisher(
        ...
        [], // Books will be populated separately if needed
        publisherEntity.user ? mapUserEntityToDomain(publisherEntity.user) : new User()
    );
...
export function mapBookEntitiesToDomain(bookEntities: BookEntity[]): Book[] {
    return bookEntities ? bookEntities.map(entity => mapBookEntityToDomain(entity)) : [];
}

export function mapAuthorEntitiesToDomain(authorEntities: AuthorEntity[]): Author[] {
    return authorEntities ? authorEntities.map(entity => mapAuthorEntityToDomain(entity)) : [];
}

export function mapPublisherEntitiesToDomain(publisherEntities: PublisherEntity[]): Publisher[] {
    return publisherEntities ? publisherEntities.map(entity => mapPublisherEntityToDomain(entity)) : [];
}